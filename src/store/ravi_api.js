...
const baseQueryWithErrorHandling = async (args, api, extraOptions) => {
  ...
  // Log the actual request URL being constructed
  if (args.url) {
    const constructedUrl = `${API_BASE_URL}${args.url.startsWith('/') ? args.url.substring(1) : args.url}`;
    console.log('🔗 Constructed Request URL:', constructedUrl);
    console.log('🔗 Request Method:', args.method || 'GET');
    console.log('🔗 Request Body:', args.body ? JSON.stringify(args.body, null, 2) : 'No body');
  }
...
export const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    // 18. Save Contact (Delegate)
    saveDelegateContact: builder.mutation({
      query: (contactData) => {
        // Ensure proper URL construction - remove leading slash from endpoint if base URL ends with slash
        const endpoint = API_ENDPOINTS.DELEGATE_SAVE_CONTACT.startsWith('/') 
          ? API_ENDPOINTS.DELEGATE_SAVE_CONTACT.substring(1) 
          : API_ENDPOINTS.DELEGATE_SAVE_CONTACT;
        return {
          url: endpoint, // Use cleaned endpoint without leading slash
          method: 'POST',
          body: contactData,
        };
      },
    ...
    // 19. Get Contacts (Delegate)
    getContacts: builder.query({
      query: () => API_ENDPOINTS.DELEGATE_CONTACTS,
