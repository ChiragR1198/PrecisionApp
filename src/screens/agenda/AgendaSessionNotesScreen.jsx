...
export const AgendaSessionNotesScreen = () => {
  const params = useLocalSearchParams();
  const agendaId = params?.agendaId ? String(params.agendaId) : '';
  const sessionTitle = params?.sessionTitle ? String(params.sessionTitle) : '';

  const [draftNotes, setDraftNotes] = useState('');
  const [pickedImage, setPickedImage] = useState(null);
  const [isImagePickerModalVisible, setIsImagePickerModalVisible] = useState(false);
  // Pending picker source ('camera' | 'gallery') queued while the modal dismisses.
  ...
  const handleBack = useCallback(() => {
    ...
    if (returnTo === 'agenda-detail' && agendaId) {
      const detailParams = { agendaId };
      if (params?.eventId) detailParams.eventId = String(params.eventId);
      for (const key of [
        'initialTitle',
        'initialTime',
        'initialDescription',
        ...
        if (params?.[key] != null && String(params[key]).trim() !== '') {
          detailParams[key] = String(params[key]);
        }
      }
      router.replace({
  ...
  // iOS-only: fires after the Modal has fully animated away. Safe moment to
  // present another native sheet (camera permission dialog / picker).
  ...
  const handlePickImage = useCallback(
    (source) => {
      ...
      setIsImagePickerModalVisible(false);
      if (Platform.OS !== 'ios') {
        // Android: Modal has no onDismiss. Wait a tick for unmount, then launch.
        setTimeout(() => {
          const pending = pendingPickerSourceRef.current;
          pendingPickerSourceRef.current = null;
  ...
  const performDelete = useCallback(
    async (noteId) => {
      ...
      try {
        const res = await deleteNote({ id: noteId, agendaId }).unwrap();
  ...
  const listHeader = (
    <View style={styles.formBlock}>
      ...
      <TouchableOpacity
        style={[styles.saveBtn, (isSaving || !agendaId) && styles.saveBtnDisabled]}
        onPress={onSave}
        disabled={isSaving || !agendaId}
        activeOpacity={0.9}
      >
        {isSaving ? (
  ...
  if (!agendaId) {
    return (
      <SafeAreaView style={styles.safe} edges={['bottom']}>
        <Header title="Session notes" leftIcon="back" onLeftPress={handleBack} />
        <View style={styles.centerMsg}>
          <Text style={styles.muted}>No session selected.</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      <Header
        title="Session notes"
        subtitle={sessionTitle ? sessionTitle.slice(0, 80) : undefined}
        leftIcon="back"
